  ...
  serial_driver_{new drivers::serial_driver::SerialDriver(*owned_ctx_)}
{
  ...
  detector_param_client_ = std::make_shared<rclcpp::AsyncParametersClient>(this, "box_detector");
  ...
  // Create Subscription
  target_sub_ = this->create_subscription<auto_aim_interfaces::msg::Boxs>(
    "/detector/boxs", rclcpp::SensorDataQoS(),
  ...
  while (rclcpp::ok()) {
    try {
      ...
      if (header[0] == 0x5A) {
        ...
        bool crc_ok =
          crc16::Verify_CRC16_Check_Sum(reinterpret_cast<const uint8_t *>(&packet), 8U/*sizeof(packet)*/);
        if (crc_ok) {
...
void RMSerialDriver::sendData(const auto_aim_interfaces::msg::Target::SharedPtr msg)
{
  ...
  try {
    ...
    // packet.dz = msg->dz;
    crc16::Append_CRC16_Check_Sum(reinterpret_cast<uint8_t *>(&packet), 32U);
